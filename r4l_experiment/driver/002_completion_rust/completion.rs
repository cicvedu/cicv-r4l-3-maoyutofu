...
static COMPLETION_LOCK: Mutex<Option<Pin<Box<Completion>>>> = unsafe { Mutex::new(None) };
...
impl file::Operations for CompletionFile {
    ...
    fn read(
        ...
        let completion = COMPLETION_LOCK.lock().deref().as_ref().unwrap().get();
...
impl kernel::Module for Completiondev {
    fn init(name: &'static CStr, module: &'static ThisModule) -> Result<Self> {
        ...
        unsafe {
            bindings::init_completion(inner.get());
        };
        let mut completion = COMPLETION_LOCK.lock();
